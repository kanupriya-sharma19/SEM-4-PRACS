...
void cscan(int movements[], int numMovements, int currentPosition, int direction) {
    int totalSeekTime = 0;
    ...
    if (direction == 1) {  // Upward direction
        ...
        // After reaching the end, jump to the lowest (0)
        totalSeekTime += abs(currentPosition - 199);  // Move to 199
        currentPosition = 199;

        // Now jump to 0 and move upwards
        totalSeekTime += abs(currentPosition - 0);  // Jump to 0
        currentPosition = 0;

        // Now move upwards from the start
        for (int j = 0; j < i; j++) {
            totalSeekTime += abs(currentPosition - movements[j]);
            currentPosition = movements[j];
        }
        ...
        // After reaching 0, jump to the highest (199)
        totalSeekTime += abs(currentPosition - 0);  // Move to 0
        currentPosition = 0;

        // Now jump to 199 and move downwards
        totalSeekTime += abs(currentPosition - 199);  // Jump to 199
        currentPosition = 199;

        // Now move downwards from the highest
        for (int j = numMovements - 1; j > i; j--) {
            totalSeekTime += abs(currentPosition - movements[j]);
            currentPosition = movements[j];
        }
    }
...
int main() {
    ...
    cscan(movements, numMovements, currentPosition, direction);
